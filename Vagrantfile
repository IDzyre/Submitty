...
def mount_folders(config, mount_options)
  ...
  owner = 'root'
  group = 'vagrant'
  config.vm.synced_folder '.', '/usr/local/submitty/GIT_CHECKOUT/Submitty', create: true, owner: owner, group: group, mount_options: mount_options, smb_host: '10.0.2.2', smb_username: 'runner', smb_pasword: 'runner'
  ...
  optional_repos.each {|repo|
    ...
    if File.directory?(repo_path)
      config.vm.synced_folder repo_path, "/usr/local/submitty/GIT_CHECKOUT/" + repo, owner: owner, group: group, mount_options: mount_options, smb_host: '10.0.2.2', smb_username: 'runner', smb_pasword: 'runner'
    end
  }
end
